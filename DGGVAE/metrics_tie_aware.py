...
def evaluate(
    ...
    user_test, item_test = [], []
    members_u = [
        torch.LongTensor(group_member_dict[group_id]).to(device)
        for group_id in list(group_member_dict.keys())
    ]

    members_i = [
        torch.LongTensor(group_item_dict[group_id]).to(device)
        for group_id in list(group_item_dict.keys())
    ]
    ...
    if type_m == "group":
        _, predictions = model(
            None, users_var, items_var, items_var, members_u, members_i, "eval", "group"
        )
    elif type_m == "user":
        _, predictions = model(
            users_var, None, items_var, items_var, None, None, "eval", "user"
        )
