...
def build_group_graph2(group_data_u, group_data_i, num_groups, k):
    ...
    for i in range(num_groups):
        group_u_a = set(group_data_u[i])
        group_i_a = set(group_data_i[i])
        for j in range(i + 1, num_groups):
            group_u_b = set(group_data_u[j])
            group_i_b = set(group_data_i[j])
            overlap_u = group_u_a & group_u_b
            overlap_i = group_i_a & group_i_b
            union_u = group_u_a | group_u_b
            union_i = group_i_a | group_i_b
            # weight computation
            matrix[i][j] = float(0.5 * (len(overlap_u) / len(union_u) + len(overlap_i) / len(union_i)))
            matrix[j][i] = matrix[i][j]
    # Only keep top-k similarities for each group
    for i in range(num_groups):
        # Get indices that would sort the array, then take the last k indices
        topk_indices = np.argsort(matrix[i])[-k:]

        # Create a mask where only topk indices are True
        mask = np.zeros(num_groups, dtype=bool)
        mask[topk_indices] = True

        # Set all non-topk elements to 0
        matrix[i, ~mask] = 0

    # Ensure the diagonal is 1 for self similarity
...
def build_group_graph1(group_data, num_groups, k):
    """Return group-level graph (**a weighted graph** with weights defined as ratio of common members and items)"""
    matrix = np.zeros((num_groups, num_groups))
...
def build_hyper_graph(group_member_dict, group_train_path, num_users, num_items, num_groups, group_item_dict=None):
    ...
    if group_item_dict is None:
        ...
        group_item_dict_pure = defaultdict(list)
        ...
        for line in open(group_train_path, 'r').readlines():
            ...
            if len(contents) > 2:
                ...
                if rating > 0:
                    ...
                    group_item_dict_pure[group].append(item)
            else:
    ...
    group_data_u = [group_member_dict[group_id] for group_id in range(num_groups)]
    group_data_i = [group_item_dict[group_id] for group_id in range(num_groups)]
    ...
    return user_hyper_graph, item_hyper_graph, full_hyper_graph, group_data, group_data_u, group_data_i, group_item_dict_pure
