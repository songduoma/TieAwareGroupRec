from datautil import load_rating_file_to_matrix, load_rating_file_to_list, load_negative_file, \
    load_group_member_to_dict, build_hyper_graph, build_group_graph1, build_group_graph2, convert_sp_mat_to_sp_tensor
import torch
...
class GroupDataset(object):
    def __init__(self, user_path, group_path, num_negatives, dataset="Mafengwo", k=20):
        ...
        self.user_hyper_graph, self.item_hyper_graph, self.full_hg, group_data, group_data_u, group_data_i, self.group_item_dict = build_hyper_graph(
            self.group_member_dict, group_path + "Train.txt", self.num_users, self.num_items, self.num_groups)
        # Group-level graph
        # w distribution sensitive
        # self.overlap_graph = build_group_graph1(group_data, self.num_groups, k)
        # w/o distribution sensitive
    ...
    def get_edges(self):
        ...
        cols = []
        data = []
        for (i, j), value in self.group_train_matrix.items():
            rows.append(i)
            cols.append(j)
            data.append(value)
        interaction_matrix = coo_matrix((data,(rows, cols)), shape=(self.num_groups, self.num_items))
