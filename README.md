...
To inspect archived results without retraining, use each method's `print.py` and `print_aware.py` on the bundled log folders (for `ITR`, use `AlignGroup/print.py ITR` and `AlignGroup/print_aware.py ITR`).
