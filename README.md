...
## 6.1 Mode A (fast): Reproduce from archived logs/results in this artifact

This mode reproduces paper numbers directly from archived outputs, with no retraining.

### AlignGroup

```bash
cd AlignGroup
python print.py log_original
python print_aware.py log_original
python print_aware.py log_revised
cd ..
```
...
`ITR` has archived logs named `output_*_original_seed*.log` and `output_*_revised_seed*.log`.
Use the shared parser:

```bash
python AlignGroup/print.py ITR
python AlignGroup/print_aware.py ITR
```

### DHMAE

`DHMAE` archived results are provided as seed-wise summary text files:

- `DHMAE/metrics_tie_aware_original_seed0.txt`
- `DHMAE/metrics_tie_aware_original_seed1.txt`
- `DHMAE/metrics_tie_aware_original_seed2.txt`
- `DHMAE/metrics_tie_aware_revised_seed0.txt`
- `DHMAE/metrics_tie_aware_revised_seed1.txt`
- `DHMAE/metrics_tie_aware_revised_seed2.txt`

### Baselines used in corrected comparison

```bash
cd Baseline/AGREE      && python print.py log_original && python print_aware.py log_revised && cd ../..
cd Baseline/GroupIM    && python print.py log          && python print_aware.py log         && cd ../..
cd Baseline/HCR        && python print.py log_original && python print_aware.py log_revised && cd ../..
cd Baseline/HyperGroup && python print.py log_original && python print_aware.py log_revised && cd ../..
cd Baseline/HHGR       && python print.py log          && python print_aware.py log         && cd ../..
cd Baseline/CubeRec    && python print.py log          && python print_aware.py log         && cd ../..
```

## 6.2 Mode B (full rerun): Retrain and regenerate logs
...
- Use three seeds (e.g., `0 1 2`) and average.
- Store outputs separately for `original` vs `revised` runs.

### 6.2.1 ConsRec (WWW2023ConsRec)
...
## 6.4 Temperature-scaled BPR (τ-BPR) experiment (Table 6 / Figure 2)
