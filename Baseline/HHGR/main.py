...
logging.info("Pre-Training on user-item interactions...")
...
for epoch in range(args.epoch):
    ...
    logging.info(f"[Epoch {epoch}] time {elapsed:4.2f}s, loss {np.mean(losses):4.4f}")
logging.info("Pre-Training finish!\n")

# User自监督训练，优化的是两个用户的HyperGCN参数
logging.info("Training on user-level self-supervised...")
...
for epoch in range(args.epoch):
    ...
    elapsed = time.time() - start_time
    logging.info(f"[Epoch {epoch}] time {elapsed:4.2f}s, loss {np.mean(user_level_loss):4.4f}")
logging.info("Training on user-level finish!\n")
...
logging.info("Training on group-level hypergraph learning...")
...
for epoch in range(args.group_epoch):
    ...
    for batch_idx, (g, pi_ni) in enumerate(dataset.get_group_dataloader(args.batch_size)):
        if batch_idx % 100 == 0:
            logging.info(f"batch {batch_idx}, {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        optimizer_gr.step()
    elapsed = time.time() - start_time
    logging.info(f"[Epoch {epoch}] time {elapsed:4.2f}s, loss {np.mean(group_level_loss):4.4f}")
    logging.info("Training on group-level finish!\n")
...
metric_evaluators = [
    ...
    ("metrics_first", metric.evaluate_first),
    ("metrics_last", metric.evaluate_last),
    ("metrics_tie_aware", metric.evaluate_tie_aware),
]
for metric_name, evaluator in metric_evaluators:
    hits, ndcgs = evaluator(
        ...
        pred_score_stats_prefix=f"[Evaluate] [user evaluate] [{metric_name}]",
        log_fn=logging.info,
    )
    logging.info(f"[Evaluate] User [{metric_name}] Hit@[1, 5, 10] {hits}, NDCG@[1, 5, 10] {ndcgs}")
    ...
    hits, ndcgs = evaluator(
        ...
        'group',
        print_pred_score_stats=True,
        pred_score_stats_prefix=f"[Evaluate] [group evaluate] [{metric_name}]",
        log_fn=logging.info,
    )
    logging.info(f"[Evaluate] Group [{metric_name}] Hit@[1, 5, 10] {hits}, NDCG@[1, 5, 10] {ndcgs}")
