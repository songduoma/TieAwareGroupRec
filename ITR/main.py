...
if __name__ == "__main__":
    ...
    for epoch_id in range(args.epoch+1):
        ...
        if (epoch_id + 1) % 100 == 0:
            ...
            for metric_name, evaluator in metric_evaluators:
                ...
                print(
                    f"[Epoch {epoch_id}] Group [{metric_name}], Hit@{args.topK}: {group_hits}, NDCG@{args.topK}: {group_ndcgs}"
                )
                ...
                print(
                    f"[Epoch {epoch_id}] User [{metric_name}], Hit@{args.topK}: {user_hits}, NDCG@{args.topK}: {user_ndcgs}"
                )
            print()
    ...
    for metric_name, evaluator in metric_evaluators:
        ...
        print(f"[Epoch {epoch_id}] Group [{metric_name}], Hit@{args.topK}: {group_hits}, NDCG@{args.topK}: {group_ndcgs}")
        ...
        print(f"[Epoch {epoch_id}] User [{metric_name}], Hit@{args.topK}: {user_hits}, NDCG@{args.topK}: {user_ndcgs}")
    print()
