...
if __name__ == "__main__":
    ...
    for epoch_id in range(args.epoch+1):
        ...
        if (epoch_id + 1) % 100 == 0:
            ...
            print(f"[Epoch {epoch_id}] Group [metrics], Hit@{args.topK}: {group_hits}, NDCG@{args.topK}: {group_ndcgs}")
            ...
            print(f"[Epoch {epoch_id}] Group [metrics_after], Hit@{args.topK}: {group_hits_after}, NDCG@{args.topK}: {group_ndcgs_after}")
            ...
            print(f"[Epoch {epoch_id}] User [metrics], Hit@{args.topK}: {user_hits}, NDCG@{args.topK}: {user_ndcgs}")
            ...
            print(f"[Epoch {epoch_id}] User [metrics_after], Hit@{args.topK}: {user_hits_after}, NDCG@{args.topK}: {user_ndcgs_after}")
            print()
